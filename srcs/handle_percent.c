...
/*   Updated: 2020/05/16 03:08:42 by mbouzaie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_va_fncts(int (*print_va_fncts[8])(va_list *, t_flag flag))
{
...
{
	int		tmp;
	int		(*print_va_fncts[8])(va_list *, t_flag flag);
	char	*tab;
	t_flag	flag;

	fill_va_fncts(print_va_fncts);
	tab = (char[9]) {'s', 'c', 'd', 'p', 'x', 'X', 'u', 'i', 0};
	*psize += *pos;
